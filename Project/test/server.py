...
class MyProtocol(asyncio.Protocol):
    ...
    def data_received(self, data_bytes):
        try:
            ...
            logging.error('Data received but decoding failed.')
            return
    ...
    def respond(self, command):
        ...
        if (len(argv) == 6) and (valid_PEER(argv[0])):
            try:
                self.handle_PEER(argv[1:], argv[0][4:]); return
    ...
    def handle_IAMAT(self, info, command):
        ...
        self.update_database(clt_id,new_data,None)
    ...
    def handle_PEER(self, info, src_srv):
        ...
        self.update_database(clt_id, new_data, src_srv)
        
    def update_database(self, clt_id, new_data, src_srv):
        ...
        if (history != new_data):
            ...
            self.loop.create_task(self.prop(new_data_msg.encode(), src_srv))
        else:
            logging.debug('Already in database:\n\t{0}:{1}'.format(clt_id,history))
        
    async def prop(self, data_bytes, src_srv):
        for peer_srv in srv2peers[self.srv]:
            # do not send back to source server
            if peer_srv == src_srv:
                logging.debug('Skip source of data, {}'.format(src_srv)); continue
